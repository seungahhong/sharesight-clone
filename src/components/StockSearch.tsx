...
export default function StockSearch({ onSelect }: { onSelect: (stock: KoreanStockPriceInfo) => void }) {
                ...
                <div className="relative mt-1">
                    <div className="relative w-full cursor-default overflow-hidden rounded-lg bg-white text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
                        <Combobox.Input
                            ...
                            displayValue={(stock: KoreanStockPriceInfo) => stock ? `${stock.itmsNm} (${stock.srtnCd})` : ''}
                                    ...
                                    >
                                        {({ selected, active }) => (
                                            <>
                                                ...
                                                <span className={`block text-xs ${active ? 'text-teal-100' : 'text-gray-500'}`}>
                                                    {parseInt(stock.clpr).toLocaleString()}원 ({stock.fltRt}%)
                                                </span>
                                                {selected ? (
                                                    <span

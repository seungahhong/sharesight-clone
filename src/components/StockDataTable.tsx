            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        ...
                        <TableHead className="text-right whitespace-nowrap">전일대비</TableHead>
                        <TableHead className="text-right whitespace-nowrap">등락률</TableHead>
                        <TableHead className="text-right whitespace-nowrap">거래량</TableHead>
                    ...
                    {data.length > 0 ? (
                        data.map((row) => (
                            <TableRow key={`${row.code}-${row.date}`}>
                                ...
                                <TableCell className="text-right whitespace-nowrap">{row.price}원</TableCell>
                                <TableCell
                                    className={`text-right font-medium whitespace-nowrap ${parseFloat(row.change) > 0 ? 'text-red-600' :
                                ...
                                <TableCell
                                    className={`text-right font-medium whitespace-nowrap ${parseFloat(row.changeRate) > 0 ? 'text-red-600' :
                                            parseFloat(row.changeRate) < 0 ? 'text-blue-600' : 'text-gray-900'
                                        }`}
                                >
                                    {parseFloat(row.changeRate) > 0 ? '+' : ''}{row.changeRate}%
                                </TableCell>
                                <TableCell className="text-right whitespace-nowrap">{row.volume}</TableCell>

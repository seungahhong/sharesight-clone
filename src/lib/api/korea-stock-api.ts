...
export async function searchKoreanStocks(
  ...
  const url = `${BASE_URL}/getStockPriceInfo?serviceKey=${API_KEY}&resultType=json&numOfRows=100&likeItmsNm=${encodeURIComponent(
    query
  )}&beginBasDt=${beginBasDt}&endBasDt=${endBasDt}`;
  ...
  try {
    ...
    rawList.forEach((item) => {
      if (
        !latestMap.has(item.srtnCd) ||
        item.basDt > latestMap.get(item.srtnCd)!.basDt
      ) {
...
export async function getKoreanStockQuote(
  stockCode: string
): Promise<KoreanStockPriceInfo | null> {
